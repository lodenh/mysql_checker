...
function checker(num_SELECT_req, num_FROM_req, num_JOIN_req, num_WHERE_req, num_AND_req, num_OR_req, columns_req, tables_req, on_col, where_cond, order_by){
  ...
  #-----------------------------------------------------------------------------------------------------------------
  # b4_where checks for required number of WHERE, AND, and OR statements and that WHERE conditions match what is required.
  #
  {if (result_b0[0] == 0){
    {result_b4[0] = 0;}
    {b4_where(debug_mode, result_b4, num_WHERE_req, num_AND_req, num_OR_req, where_cond);}
